...
const SOLUTION_ITEMS = [
  ...
  },
  {
    id: 4,
    label: 'Monetization',
    description: 'Charge for events, subscriptions, and accept donations.',
  ...
  return (
    <>
      <div className='mt-24 mb-5 text-primary mx-auto flex justify-center items-center gap-2 bg-primary-foreground py-2 px-4 rounded-lg'>
        <ThumbsUp size={18} />
        <p className='font-medium'>Your Best Event Manage Platform</p>
      </div>
      <div className='text-center'>
        <p className='text-5xl font-semibold mb-1'>Our Solution For Your Business</p>
        <p className='text-muted-foreground'>
          Bring your plan to reality. We give the resources and support you need to make your event truly distinctive.
        </p>
        <p className='text-muted-foreground'>Let's plan your own event!</p>
      </div>
      <div className='flex justify-between items-center gap-4 mt-14 mb-10'>
        {SOLUTION_ITEMS.map((item) => (
          <Card key={item.id} className='glass p-4 w-full min-h-48 flex flex-col'>
